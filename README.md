...
端末内の生成AI **Gemma 4（E4B）** が音声認識と翻訳を行い、内蔵音声合成で読み上げます。
...
```
マイク(本体/USB) → 録音 → Gemma 4 E4B(音声→原文＋翻訳) → 端末内TTSで発声
```

## ⬇ ダウンロード（Android）
- **最新リリース**: https://github.com/batake321/voicetrans-android/releases/latest
- **APK直リンク**: [voicetrans-0.9.2.apk](https://github.com/batake321/voicetrans-android/releases/download/v0.9.0/voicetrans-0.9.2.apk)
...
- 約 **3.7GB** をダウンロード（進捗％・残り時間を表示、**Wi-Fi推奨**、途中で切れても**自動で再開**）。
...
## 翻訳モデルとライセンス（重要）
本アプリは Google の **Gemma 4** を利用します。利用は
...
モデルは Releases に分割（GitHubの2GB/ファイル制限のため約1.8GB×2）して置かれ、アプリが自動で結合します。
